...
import { collection, onSnapshot, doc, deleteDoc, updateDoc, query, where, addDoc } from "firebase/firestore";
...
// IMPORT KOMPONEN MODAL UNTUK KEMENTERIAN
...
export default function DashboardKementerian() {
  ...
  const [formProker, setFormProker] = useState({ 
    nama: "", tujuan: "", sasaran: "", kpi: "", pj: "", anggaran: "", waktu_pelaksanaan: "", scope: "" 
  });
  ...
  const [dashboardModalContent, setDashboardModalContent] = useState<"surat" | "keuangan" | "proker" | "pengurus" | null>(null);
  ...
  // FIX TYPE ERROR: Pastikan hasil localStorage selalu string murni
  ...
  useEffect(() => {
    // Validasi Akses: Hanya untuk akun dengan role 'kementerian'
    if (typeof window !== 'undefined' && (!userRole || userRole === "bph")) {
      alert("Akses ditolak! Halaman ini khusus Kementerian.");
      router.push("/login");
    }
  }, [userRole, router]);

  useEffect(() => {
    if (!userRole || userRole === "bph") return;

    const timer = setInterval(() => setCurrentTime(new Date().toLocaleTimeString("id-ID")), 1000);

    // Ambil Surat Masuk KHUSUS KEMENTERIAN INI
    const unsubSM = onSnapshot(query(collection(db, "surat_masuk"), where("scope", "==", userRole)), snap => {
        setSuratMasuk(snap.docs.map(doc => ({ id: doc.id, ...doc.data() })));
        setTotalSurat(prev => prev + snap.size);
    });

    // Ambil Surat Keluar KHUSUS KEMENTERIAN INI
    const unsubSK = onSnapshot(query(collection(db, "surat_keluar"), where("scope", "==", userRole)), snap => {
        setSuratKeluar(snap.docs.map(doc => ({ id: doc.id, ...doc.data() })));
        setTotalSurat(prev => prev + snap.size);
    });
    
    // Ambil Keuangan KHUSUS KEMENTERIAN INI
    const unsubKeu = onSnapshot(query(collection(db, "keuangan"), where("scope", "==", userRole)), snap => {
        let total = 0;
        const sortedData = snap.docs.map(doc => {
            const d = doc.data();
    ...
    // Ambil Proker (KHUSUS MILIK MEREKA SAJA)
    const unsubProker = onSnapshot(query(collection(db, "program_kerja"), where("scope", "==", userRole)), snap => {
    ...
    // Ambil Presensi (KHUSUS YANG DIBUAT KEMENTERIAN INI)
    const unsubPresensi = onSnapshot(query(collection(db, "presensi"), where("pembuat", "==", userRole)), snap => {
       const data = snap.docs.map(doc => ({ id: doc.id, ...doc.data() })).sort((a:any, b:any) => (b.createdAt || 0) - (a.createdAt || 0));
       setDaftarPresensi(data);
    });

    // Data Beranda dari BPH (Read-Only)
    const unsubWeb = onSnapshot(doc(db, "pengaturan", "beranda"), (docSnap) => {
      if (docSnap.exists()) {
        ...
        if (data.dataPengurus && Array.isArray(data.dataPengurus)) {
            // Filter hanya pengurus yang sesuai dengan nama kementerian ini
            const myPengurus = data.dataPengurus.filter((p:any) => p.lembaga.toLowerCase().includes((userName || "").toLowerCase()));
            setDataPengurus(myPengurus);
        } else {
            setDataPengurus([]);
        }
      }
    ...
    return () => { clearInterval(timer); unsubSM(); unsubSK(); unsubKeu(); unsubProker(); unsubPresensi(); unsubWeb(); };
  }, [userRole, userName]); 
  ...
  const sortedKeluar = [...suratKeluar]
  ...
  const exportKeuanganToExcel = (kategoriStr: string) => {
    let rawData = dataKeuangan.filter(k => k.cat === kategoriStr);
    let formattedData;
    ...
    XLSX.writeFile(wb, `Laporan_Keuangan_${kategoriStr}_Kementerian.xlsx`);
  };
    ...
    try {
      if (editProkerId) {
         ...
         await addDoc(collection(db, "program_kerja"), { 
             ...
             scope: userRole, // FIX: Set otomatis scope kementerian
             createdAt: Date.now()
         });
         alert("Program Kerja berhasil disimpan!");
      }
      
      setFormProker({ nama: "", tujuan: "", sasaran: "", kpi: "", pj: "", anggaran: "", waktu_pelaksanaan: "", scope: "" });
  ...
  const handleSavePresensi = async (e: React.FormEvent) => {
    ...
    try {
      await addDoc(collection(db, "presensi"), {
        ...formPresensi,
        createdAt: Date.now(),
        pembuat: userRole // FIX: Agar kementerian hanya melihat yang dia buat
      });
  ...
  // HANYA MENGAMBIL PENGURUS YANG SESUAI DENGAN KEMENTERIAN INI
  ...
  // MENGELOMPOKKAN PROKER BERDASARKAN USER YANG LOGIN (Bukan listKementerian)
  const groupedProkers = daftarSemuaProker.reduce((acc, curr) => {
      const scopeLabel = userName || "Program Kerja Kementerian";
  ...
  const renderDashboardModal = () => {
    ...
    if (dashboardModalContent === "surat") {
      content = (
        <div>
          <h5 className="fw-bold mb-3 border-bottom pb-2">Rincian Total Surat ({suratMasuk.length + suratKeluar.length})</h5>
          <div className="row g-3 mb-4">
            <div className="col-6">
              <div className="p-3 bg-light rounded-3 border text-center">
                <div className="fs-3 fw-bold text-dark">{suratMasuk.length}</div>
                <div className="small text-muted text-uppercase">Surat Masuk</div>
              </div>
            </div>
            <div className="col-6">
              <div className="p-3 bg-light rounded-3 border text-center">
                <div className="fs-3 fw-bold text-dark">{suratKeluar.length}</div>
        ...
        <div>
          ...
          <div className="table-responsive">
            <table className="table table-sm align-middle text-nowrap">
              <tbody>
                {dataKeuangan.slice(0, 5).map((k, idx) => (
                  <tr key={idx}>
      ...
      content = (
        <div>
          <h5 className="fw-bold mb-3 border-bottom pb-2">Program Kerja Kita</h5>
      ...
      content = (
        <div>
          <h5 className="fw-bold mb-3 border-bottom pb-2">Anggota Pengurus</h5>
          <div className="row g-3">
             <div className="col-12">
               <div className="p-3 border rounded-3 bg-light d-flex justify-content-between align-items-center">
                  <span className="fw-bold text-dark text-uppercase" style={{fontSize: "0.85rem", letterSpacing: "0.5px"}}>{userName}</span>
                  <span className="badge bg-dark rounded-pill px-3">{dataPengurus.length} Orang</span>
               </div>
             </div>
             {dataPengurus.length === 0 && <p className="text-muted small w-100 text-center py-4">Data pengurus belum disinkronisasi oleh BPH.</p>}
          </div>
        </div>
      );
    }
    ...
    return (
      <div className="modal-backdrop" style={{position: "fixed", top: 0, left: 0, width: "100vw", height: "100vh", background: "rgba(15,23,42,0.6)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", backdropFilter: "blur(5px)"}}>
        <div className="bg-white rounded-4 shadow-lg overflow-hidden animate-fade-in-up" style={{width: "95%", maxWidth: "600px", maxHeight: "85vh", display: "flex", flexDirection: "column"}}>
  ...
  return (
    <div>
      ...
      <aside className={`sidebar ${isSidebarOpen ? "active" : ""}`}>
        <div className="sidebar-brand">
          ...
          <div className="d-flex flex-column">
            ...
            <span style={{ fontSize: "0.75rem", color: "#64748b", fontWeight: 600 }}>{userName || "Kementerian"}</span>
          </div>
        </div>
        ...
        <ul className="sidebar-menu">
          ...
          <li className="sidebar-heading">Administrasi</li>
          <li><a className={`nav-link ${activeMenu === "admin_surat" ? "active" : ""}`} onClick={() => { setActiveMenu("admin_surat"); setIsSidebarOpen(false); }}><i className="fas fa-envelope"></i> <span>Surat Kementerian</span></a></li>
          <li><a className={`nav-link ${activeMenu === "admin_keuangan" ? "active" : ""}`} onClick={() => { setActiveMenu("admin_keuangan"); setIsSidebarOpen(false); }}><i className="fas fa-wallet"></i> <span>Keuangan & Laporan</span></a></li>
          ...
          <li className="sidebar-heading">Data & Informasi</li>
          <li><a className={`nav-link ${activeMenu === "kepengurusan" ? "active" : ""}`} onClick={() => { setActiveMenu("kepengurusan"); setIsSidebarOpen(false); }}><i className="fas fa-sitemap"></i> <span>Anggota Pengurus</span></a></li>
          <li><a className={`nav-link ${activeMenu === "proker" ? "active" : ""}`} onClick={() => { setActiveMenu("proker"); setIsSidebarOpen(false); }}><i className="fas fa-calendar-check"></i> <span>Program Kerja Kita</span></a></li>

          <li className="sidebar-heading">Smart Tools</li>
      ...
      <header className="main-header d-flex justify-content-between align-items-center px-4 px-lg-5">
        ...
        <div className="d-flex align-items-center gap-3">
          <div className="text-end d-none d-md-block">
            <h6 className="m-0 fw-bolder text-dark" style={{ fontSize: "0.95rem" }}>{userName}</h6>
            <small className="text-muted fw-500" style={{ fontSize: "0.75rem" }}>DEMA UIN MALANG</small>
          </div>
          <div className="rounded-circle bg-primary text-white d-flex align-items-center justify-content-center fw-bold shadow-sm" style={{ width: "40px", height: "40px", border: "2px solid #e2e8f0", fontSize: "0.9rem" }}>{userName ? userName.charAt(0).toUpperCase() : "K"}</div>
        </div>
      </header>
      ...
      <div className="content-wrapper">
        ...
        {activeMenu === "dashboard" && (
          <div className="animate-fade-in-up">
            <div className="d-flex justify-content-between align-items-end mb-4">
              <div>
                <h3 className="fw-bolder m-0 text-dark" style={{ letterSpacing: "-0.5px" }}>Halo, {userName}! 👋</h3>
                <p className="text-muted small m-0 mt-1">Ini adalah ringkasan aktivitas kementerian Anda.</p>
              </div>
            </div>
            ...
            <div className="row g-4 mb-4">
              ...
              <div className="col-12 col-md-4">
                <div className="info-box" onClick={() => setDashboardModalContent("pengurus")}>
                  <div>
                    <small className="text-muted fw-bold d-block mb-1 text-uppercase" style={{ letterSpacing: "1px", fontSize: "0.75rem" }}>Anggota</small>
                    <h2 className="fw-bolder m-0 text-dark">{dataPengurus.length}</h2>
                    <span className="badge bg-light text-secondary border mt-2 px-2 py-1"><i className="fas fa-users me-1"></i>Pengurus Aktif</span>
                  </div>
            ...
            <div className="glass-card p-4 p-md-5 mt-2">
              <div className="row g-4 align-items-center position-relative" style={{ zIndex: 2 }}>
                <div className="col-lg-5 border-end border-secondary border-opacity-50 pe-lg-4">
                  ...
                  <p className="fst-italic mb-0" style={{ fontSize: "1.15rem", lineHeight: "1.6", color: "#f8fafc" }}>"{webGrandDesign || "Deskripsi Grand Design belum diatur oleh administrator BPH."}"</p>
                </div>
        ...
        {/* --- MENU: SURAT KEMENTERIAN --- */}
        {activeMenu === "admin_surat" && (
          <div className="animate-fade-in-up">
            <h4 className="fw-bolder mb-4 text-dark">Surat Kementerian</h4>
            ...
            <ul className="nav nav-pills mb-4 bg-white p-2 rounded-4 shadow-sm d-none d-md-inline-flex border">
              ...
              <li className="nav-item"><button className={`nav-link rounded-pill fw-bold px-4 text-nowrap ${adminSubTabSurat === "keluar" ? "active bg-dark text-white" : "bg-transparent text-muted"}`} onClick={() => setAdminSubTabSurat("keluar")}>Surat Keluar</button></li>
            </ul>
            ...
            {adminSubTabSurat === "masuk" && (
              <div className="card border-0 shadow-sm rounded-4 p-3 p-md-4">
                <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 pb-2 border-bottom gap-3">
                  <span className="fw-bold text-dark fs-5">Data Surat Masuk</span>
                  <div className="d-flex flex-wrap gap-2">
                    ...
                    <button className="btn btn-success btn-sm rounded-pill fw-bold px-3 shadow-sm text-nowrap" onClick={() => exportToExcel(sortedMasuk, `Surat_Masuk_${userRole}`, "Masuk")}><i className="fas fa-file-excel me-1"></i> Excel</button>
            ...
            {adminSubTabSurat === "keluar" && (
              <div className="card border-0 shadow-sm rounded-4 p-3 p-md-4">
                <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 pb-2 border-bottom gap-3">
                  <span className="fw-bold text-dark fs-5">Data Surat Keluar</span>
                  <div className="d-flex flex-wrap gap-2">
                    ...
                    <button className="btn btn-success btn-sm rounded-pill fw-bold px-3 shadow-sm text-nowrap" onClick={() => exportToExcel(sortedKeluar, `Surat_Keluar_${userRole}`, "Keluar")}><i className="fas fa-file-excel me-1"></i> Excel</button>
        ...
        {/* --- MENU: KEUANGAN KEMENTERIAN --- */}
        {activeMenu === "admin_keuangan" && (
          <div className="animate-fade-in-up">
            <h4 className="fw-bolder mb-4 text-dark">Keuangan & Laporan</h4>
            ...
            {(adminSubTabKeu === "dipa" || adminSubTabKeu === "intern" || adminSubTabKeu === "extern") && (() => {
              ...
              const rawData = dataKeuangan.filter(k => k.cat === catLabel);
              let tempSaldo = 0;
            ...
            {adminSubTabKeu === "kepanitiaan" && (() => {
               const kepanitiaanData = dataKeuangan.filter(k => k.cat === "Kepanitiaan");
          ...
          <div className="animate-fade-in-up">
            ...
            <div className="card border-0 shadow-sm rounded-4 p-3 p-md-4 bg-white mb-4">
              <div className="d-flex justify-content-between align-items-center mb-4 border-bottom pb-2">
                <span className="fw-bold text-dark fs-5">Daftar Link Presensi Anda</span>
          ...
          <div className="animate-fade-in-up">
            <div className="d-flex flex-column flex-sm-row justify-content-between align-items-sm-end mb-4 gap-3">
              <div>
                ...
                <p className="text-muted small m-0 mt-1">Daftar pengurus kementerian Anda saat ini.</p>
              </div>
            ...
            {dataPengurus.length === 0 ? (
              <div className="card border-0 shadow-sm rounded-4 p-5 text-center">
                ...
                <p className="text-muted small">BPH belum mengupload file Excel struktur pengurus atau tidak ada nama kementerian Anda di data BPH.</p>
              </div>
            ) : (
              <div className="card border-0 shadow-sm rounded-4 mb-4 overflow-hidden">
                <div className="bg-light border-bottom p-3 d-flex align-items-center">
                  ...
                  <h5 className="fw-bolder m-0 text-dark text-uppercase" style={{ letterSpacing: "1px" }}>{userName}</h5>
                </div>
                <div className="card-body bg-white p-4">
                  <div className="row g-3">
                    {dataPengurus.map((m, idx) => (
        ...
        {/* --- MENU: PROGRAM KERJA KEMENTERIAN --- */}
        {activeMenu === "proker" && (
          <div className="animate-fade-in-up">
            ...
            {prokerTab === "umum" && (
              <div className="card border-0 shadow-sm rounded-4 p-3 p-md-4 bg-white animate-fade-in-up">
                <div className="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center mb-4 border-bottom pb-2 gap-3">
                  <span className="fw-bold text-dark fs-5">Daftar Program Kerja Kita</span>
                  <button className={`btn rounded-pill fw-bold px-3 px-md-4 shadow-sm ${showAddProker ? 'btn-light border' : 'btn-dark'}`} onClick={() => { setShowAddProker(!showAddProker); setEditProkerId(null); setFormProker({ nama: "", tujuan: "", sasaran: "", kpi: "", pj: "", anggaran: "", waktu_pelaksanaan: "", scope: userRole }); }}>
          ...
          <div className="animate-fade-in-up">
            <div className="d-flex flex-column flex-md-row align-items-md-center mb-4 pb-2 border-bottom gap-3">
              ...
              <div>
                ...
                <p className="text-muted m-0 mt-1">Kumpulan alat cerdas otomatis untuk mempercepat administrasi kementerian.</p>
              </div>
            </div>
            ...
            {aiTab === "smartletter" && (
              <div className="card border-0 shadow-sm rounded-4 p-3 p-md-5 bg-white animate-fade-in-up">
                <div className="text-center mb-4">
                  ...
                  <p className="text-secondary mb-4">Sistem AI memeriksa format teks secara langsung dari file PDF yang Anda unggah berdasarkan acuan BPH.</p>
                  ...
                  {poPptaFileUrl ? (
                    <a href={poPptaFileUrl} target="_blank" rel="noreferrer" className="badge bg-success bg-opacity-10 text-success border border-success p-2 px-3 shadow-sm text-decoration-none rounded-pill">
                      <i className="fas fa-check-circle me-1"></i> Acuan PPTA BPH Aktif
                    </a>
                  ) : (
              ...
              <div className="card border-0 shadow-sm rounded-4 p-0 overflow-hidden bg-white animate-fade-in-up">
                <div className="row g-0">
                  <div className="col-lg-3 border-end bg-light p-4" style={{ minHeight: "600px" }}>
                    ...
                    <label className="small fw-bold mb-2 text-secondary">Ketua</label>
                    ...
                    <label className="small fw-bold mb-2 text-secondary">Sekretaris</label>
      ...
      {isModalSuratOpen && <ModalTambahSurat kementerianName={userRole} tipe={tipeSurat} initialData={editDataSurat} onClose={() => { setIsModalSuratOpen(false); setEditDataSurat(null); }} />}
      {isModalKeuOpen && <ModalTambahKeuangan kementerianName={userRole} kategori={kategoriKeu} initialData={editDataKeu} onClose={() => { setIsModalKeuOpen(false); setEditDataKeu(null); }} />}
